    ...
    : in(in_), out(out_)
{
    ...
    block_size = in.size();
    buffer_ptr = in.data();
    out.reserve(block_size / 2);
}

int Encode::get()
{
    int result = 0;
    if (offset != block_size && buffer_ptr) {
        result = buffer_ptr[offset];
        ++offset;
    } else {
        result = EOF;
        in.clear();
    ...
    : in(in_), out(out_)
{
    ...
    buffer_ptr = in.data();
    out.resize(4096);
    out_buffer_ptr = out.data();
    buffer_reserved_length = 4096;
    out_offset = 0;
}
...
void Decode::push_back(const uint8_t symbol)
{
    if (out_offset < buffer_reserved_length) {
        out_buffer_ptr[out_offset++] = symbol;
    } else {
        buffer_reserved_length = buffer_reserved_length * 2;
        out.resize(buffer_reserved_length);
        out_buffer_ptr = out.data();
    }
}
